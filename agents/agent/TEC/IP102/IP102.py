    ...
    try:
        ...
        if capture_process:
            ...
            if ping_result1.returncode == 0:
                ...
                print(ping_result1.stdout)
                error = None
            else:
                print("Ping failed. Error:")
                print(ping_result1.stderr)
                error = result.stderr.strip()
            time.sleep(5)

            # Simulate network checks
            result1 = {"argum[0]": argum, "ping_result1": ping_result1.stdout}

            if ping_result2.returncode == 0:
                print("Ping successful. Results:")
                print(ping_result2.stdout)
            else:
                print("Ping failed. Error:")
                print(ping_result2.stderr)

            # Simulate network checks
            result2 = {"argum[1]": argum, "ping_result2": ping_result2.stdout}
